...
const validator = (inputDataReceived, constraints) => {
    ...
    each(keys(constraints), constraintKey => {
      ...
      if (constraintKey.match(/\^/) && dataKey.match(constraintKey)) {
        ...
        if (validatedObjWithErr) {
          let errMessage = get(validatedObjWithErr, "t[0]").replace(
            "T ",
            `${capitalize(dataKey.split(".").pop())} `,
          );
  ...
  errorsObjOfArrays = Object.assign(
    errorsObjOfArrays,
    validate(inputDataReceived, constraints),
  );
  ...
  if (!isValid) {
    errors = keys(errorsObjOfArrays).reduce((obj, error) => {
      obj[error] =
        typeof get(errorsObjOfArrays, `[${error}][0]`) === "string"
